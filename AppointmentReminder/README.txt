...
Written by Michael Williams (mrwilliams@ascension.org)

Projects and Submodules:

	AppointmentReminder2.5 Update (7/5/2017)
		*Program_Start.bat file no longer used for routine program initialization. Instead, it now passes an argument
		to the program which causes the program to find the second most recently created Centricity files from the
		Centricity_Reports folder and find screening location for the following day. This will allow someone to rerun
		the program manually the day before the screening without having to change a bunch of parameters.
			%% to use this function, the send invitation function in Redcap must also be changed or it will 
			not send the reminders out until the day of the screening.

		*The program has a new sanity check. When assembling records, it checks the date in the date time field
		(which used to only be used to get appointment time for each patient) to make sure it matches with the date 
		of the screening.

		*The program no longer locates the appropriate Centricity file for creating records based on last date modified.
		Instead, it finds the appropriate date in the file name. Last date modified is still used for deleting old files.

	AppointmentReminder2.4 Update(6/5/2017)
		*Centricity report identification was changed substantially. In the previous version, the two most recently edited (created) files in the Centricity folder were compared by size and the larger sized file was selected to pull patient data. This version loops through all files in the Centricity folder and pulls patient data from any number of files created on the same date the program is run. If multiple files contain patient data, the data from these files can all be uploaded into REDCap.

		*Patient name and location are now appropriately cased instead of using only all caps.

		*Location book data extraction has been modified to allow use of a single standard workbook for both this application and project team daily documentation needs.

		*Added filePath.py file to consolidate all file paths used in the program.

		*Removed dates for sanity checks from references_Text.py and placed in the apptRemind_Text.py file to prevent confusion about where these values are created.
	...
	AppointmentReminder2.3 is the main repository for this application (4/10/2017).
		...
		*apptRemind_Text.py is the main file from which everything is run. When triggered, it pulls data from
		...
		a json format, logs the data in the log file then submits the data to redcap via the API. It is a known
		error the logging functions do work properly. This is a global package that for whatever reason I could
		not find a way to work properly. I am sure it is an issue with the environmental varables but even with
		tests with virtual environments, it would not work. It is unessential to the function of the program.
